// dashboard/src/app/(mobile)/jamaah/calendar/page.tsx
"use client";
...
export default function JamaahCalendarPage() {
  ...
  // ✅ Fetch events dengan proper error handling
  const { data, isLoading, error } = useQuery({
    queryKey: ["jamaah-calendar", dateRange.startDate, dateRange.endDate],
    queryFn: async () => {
      console.log("📅 Fetching calendar events:", dateRange);
      const result = await jamaahSelfService.getCalendarEvents(
        dateRange.startDate,
        dateRange.endDate,
      );
        ...
        <div className="flex items-center justify-between p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Jadwal perjalanan Anda</p>
            </div>
          </div>
        ...
        <div className="p-4">
          <Card className="border-0 shadow-sm">
            <CardContent className="py-10 text-center">
              <Calendar className="h-16 w-16 mx-auto text-red-300 mb-4" />
        ...
        <>
          ...
          <div className="p-4 space-y-3">
            ...
            {selectedDateEvents.map((event: any) => (
              ...
              >
                <div
                  className={`h-1.5 ${EVENT_COLORS[event.type]?.dot || "bg-gray-400"}`}
                />
                  ...
                  .map((event: any) => (
                    ...
                    >
                      <CardContent className="p-3">
                        <div className="flex items-center gap-3">
                          <div
                            className={`h-10 w-10 rounded-xl flex items-center justify-center ${EVENT_COLORS[event.type]?.bg}`}
                          >
                            <span className="text-lg">{event.icon}</span>
                          </div>
            ...
            {!selectedDate && events.length === 0 && (
              <Card className="border-0 shadow-sm">
                <CardContent className="py-10 text-center">
                  ...
                  <p className="text-sm text-gray-500">
                    Jadwal perjalanan akan muncul setelah Anda terdaftar di
                    paket umrah
                  </p>
                </CardContent>
              </Card>
          ...
          {selectedEvent && (
            <div className="space-y-4">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {selectedEvent.dayNumber && (
                  <Badge variant="outline">
                    Hari ke-{selectedEvent.dayNumber}
                  </Badge>
                )}
              </div>
              ...
              {selectedEvent.city && (
                <div className="flex items-center gap-3 text-sm">
                  <span className="text-lg">
                    {selectedEvent.city === "MAKKAH" && "🕋"}
                    {selectedEvent.city === "MADINAH" && "🕌"}
                    {selectedEvent.city === "JEDDAH" && "✈️"}
                    {selectedEvent.city === "JAKARTA" && "🇮🇩"}
                  </span>
