...
### 3.1 Auth Hardening Flags

Konfigurasi auth minimum untuk staging/production:

```env
AUTH_RATE_LIMIT_WINDOW=15
AUTH_RATE_LIMIT_MAX=5
OTP_RATE_LIMIT_WINDOW=5
OTP_RATE_LIMIT_MAX=3
CREATE_ACCOUNT_RATE_LIMIT_WINDOW=60
CREATE_ACCOUNT_RATE_LIMIT_MAX=5
OTP_COOLDOWN_SECONDS=60
DISABLE_RATE_LIMIT=false
```

CAPTCHA Cloudflare Turnstile disiapkan opsional dan **tetap disabled** sampai akses domain/key tersedia:
...
- [ ] Guard migration lolos: `cd backend && npm run check:migrations`.
- [ ] Guard env production lolos di target env: `cd backend && npm run check:prod-env`.
- [ ] Migrasi DB sudah ditinjau + rollback plan tersedia.
- [ ] Backup database terbaru tersedia.
- [ ] Backup file upload terbaru tersedia.
- [ ] Runtime schema patch production tetap mati: `ENABLE_RUNTIME_SCHEMA_PATCH=false` atau tidak diset.
- [ ] Rate limit auth aktif: `DISABLE_RATE_LIMIT=false`.
- [ ] OTP cooldown aktif: `OTP_COOLDOWN_SECONDS=60` atau lebih ketat.
- [ ] Changelog release tersedia untuk tim terkait.

## 4.1) Database Migration Rules

Aturan ini wajib sebelum deploy production:

- Migration source of truth adalah `backend/drizzle/migrations/**` yang sudah di-review dan masuk git.
- Jalur production hanya memakai `cd backend && npm run db:migrate`.
- `npm run db:push` sengaja dibuat gagal; `npm run db:push:dev` hanya boleh dipakai untuk database lokal disposable.
- Sebelum migrate production, ambil backup database dan simpan nama file backup di evidence deploy.
- Jika production DB pernah menerima runtime patch manual, lakukan rehearsal di staging terlebih dulu dan cocokkan kolom/index sebelum migrate.
- `ENABLE_RUNTIME_SCHEMA_PATCH` tidak boleh aktif di production. Override `ALLOW_PROD_RUNTIME_SCHEMA_PATCH=true` hanya boleh dipakai untuk emergency terdokumentasi setelah backup tersedia.
...
3. Jalankan `npm run check:migrations`.
4. Jalankan `npm run check:prod-env` dengan env production target.
5. Backup database.
6. Jalankan `npm run db:migrate` bila ada migration baru.
7. Backup uploads atau pastikan backup terbaru tersedia.
8. Restart service backend.
9. Cek health endpoint dan auth endpoint.

Catatan cPanel:

- `.cpanel.yml` menggunakan `rsync --delete`, tetapi wajib mengecualikan `.env*`, `node_modules/`, `public/uploads/`, dan `backups/`.
- Folder `public/uploads` adalah runtime storage. Jangan jadikan isi folder ini sebagai source deploy dan jangan hapus saat release.
- Restart procedure final harus mengikuti setup Node.js/cPanel yang aktif di hosting dan dicatat di evidence deploy.
...
| ------------------------------------------------ | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| Migration deploy-safe                            | `cd backend && npm run check:migrations`                       | memastikan migration SQL/meta ada, tidak ignored, dan `db:push` tidak menjadi strategi production |
| Production env deploy-safe                       | `cd backend && npm run check:prod-env`                         | jalankan dengan env production target sebelum restart                                             |
...
Checks:
- migration guard: PASS|FAIL (`npm run check:migrations`)
- production env guard: PASS|FAIL (`npm run check:prod-env`)
